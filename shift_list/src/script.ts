function shiftAll<T>(origin: T[], selected: T[]) {
  const originList = origin.map((value, index) => ({ value, index }));
  const selectedList = originList.filter(({ value }) =>
    selected.includes(value)
  );
  const sortedSelectedList = selectedList.sort((a, b) => a.index - b.index);

  const isFirstItemSelected = selected.includes(origin[0]);
  const isAlreadyOrdered = sortedSelectedList.every(
    (item, i, arr) => i === 0 || item.index === arr[i - 1].index + 1
  );
